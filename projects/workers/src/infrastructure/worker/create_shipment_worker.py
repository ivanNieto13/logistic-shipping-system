...
class CreateShipmentSubscriber(bootsteps.StartStopStep):
        ...
        while not self._stopped.is_set():
            try:
                ...
                if message and message["type"] == "message":
                    self._process_message(message["data"], repo, loop)
    ...
    def _process_message(self, data: str, repo: ShipmentRepository, loop: asyncio.AbstractEventLoop) -> None:
        try:
            payload = json.loads(data)
            print(f"Income event payload: {payload}")
            use_case = SaveShipmentUseCase(repository=repo)
            result = loop.run_until_complete(use_case.execute(SaveShipment(**payload)))
